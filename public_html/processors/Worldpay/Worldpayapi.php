...
class WorldpayAPI
{
     ...
     * @param null $amount
     */
    public function captureOrder($orderCode = false, $amount)
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            Error::throwError('ip', Error::$errors['capture']['ordercode']);
        }
     ...
     * @param null $amount
     */
    public function voidOrder($orderCode = false, $amount=false)
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            Error::throwError('ip', Error::$errors['void']['ordercode']);
        }
        OrderService::cancelAuthorizedOrder($orderCode);
    }
